...
```{r}
print.data.frame(head(ChickWeight))
...
print.data.frame(head(DNase))

print.data.frame(Formaldehyde)

print.data.frame(head(Orange))
...
```{r}
print.data.frame(filter(murders, rate<=0.71))
```
...
```{r}
print.data.frame(filter(murders, state=="New York"))
```
...
```{r}
print.data.frame(filter(murders, rank<=5))
```
...
```{r}
print.data.frame(filter(murders, state%in%c("New York", "Texas")))
```
...
```{r}
print.data.frame(filter(murders, population<5000000 & region=="Northeast"))
```
...
print.data.frame(select(my_states, state, rate, rank))
```
...
```{r}
print.data.frame(murders %>% select(state, region, rate) %>% filter(rate<=0.71))
```
...
```{r}
print.data.frame(mutate(murders, rate=total/population*100000, rank=rank(-rate)) %>% select(state, rate, rank))
```
...
```{r}
print.data.frame(filter(murders, region %in% c("Northeast", "West") & rate<1) %>% select(state, rate, rank))
```
